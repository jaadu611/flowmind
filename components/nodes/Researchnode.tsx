        ...
        <div className="border-t border-gray-800 p-3 flex flex-col gap-[10px]">
          ...
          <div className="flex flex-col gap-4 border-t border-gray-800/30 pt-4 mt-2">
            <div className="flex items-center justify-between">
              ...
              <div className="flex bg-[#05080f] rounded p-0.5 border border-gray-800/50">
                <button
                  onClick={() => data.onDataChange?.({ mode: "surface" })}
                  className={`px-3 py-1 text-[9px] rounded-sm transition-all ${(data.mode ?? "deep") === "surface" ? "bg-slate-800 text-white" : "text-slate-500 hover:text-slate-300"}`}
                >
                  Surface
                </button>
                <button
                  onClick={() => data.onDataChange?.({ mode: "deep" })}
                  className={`px-3 py-1 text-[9px] rounded-sm transition-all ${(data.mode ?? "deep") === "deep" ? "bg-slate-800 text-white" : "text-slate-500 hover:text-slate-300"}`}
                >
                  Deep
                </button>
              </div>
